...
To simulate realistic user behavior, a dedicated LLM (`USER\_LLM\_MODEL`) acts as the 





simulated user. This `USER\_LLM\_MODEL` is distinct from the `TARGET\_LLM\_MODEL` (the LLM being evaluated) and the `SUMMARIZATION\_LLM\_MODEL` (used for generating summaries).
